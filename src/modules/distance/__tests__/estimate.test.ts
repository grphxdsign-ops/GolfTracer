...
describe('estimateDistance', () => {
  ...
  it(
    "uses the receding rung ('physics-fit' + recedingFit) on a 12-point " +
      'behind track',
    () => {
      // Evidence-#4 geometry: 1080x1920 portrait, 30 fps, camera 10 m
      // behind the ball at 1.6 m height, 60 deg FOV. Track generated by
      // projecting the simulator through the behind-camera pinhole.
      ...
      const d0 = 10; // DEFAULT_CAMERA_DISTANCE_M (no ball anchor)
      const hc = 1.6;
      const focalPx = portrait.width / 2 / Math.tan((Math.PI / 180) * 30);
      const flight = simulateFlight(trueLaunch);
      const rng = makeRng(42);
      const noise = () => (rng() * 2 - 1) * 1.5;
      const smoothedPath: TrackPoint[] = [];
      for (let i = 0; i < 12; i++) {
        const sim = trajectoryAt(flight.trajectory, i / portrait.fps);
        smoothedPath.push({
          timestampMs: 1000 + (i * 1000) / portrait.fps,
          x: portrait.width / 2 + noise(),
          y:
            portrait.height / 2 +
            (focalPx * (hc - sim.y)) / (d0 + sim.x) +
            noise(),
          interpolated: false,
        });
      }
      ...
      expect(estimate.method).toBe('physics-fit');
      expect(estimate.recedingFit).toBeDefined();
      expect(estimate.recedingFit!.converged).toBe(true);
      expect(estimate.confidence).toBeGreaterThan(0.3);
      expect(estimate.confidence).toBeLessThanOrEqual(0.5);
      expect(
        Math.abs(estimate.carryYards - flight.carryYards) / flight.carryYards,
      ).toBeLessThan(0.2);

      // The identical track calibrated face-on has no recession rung and no
      // converging planar fit — it must fall to the honest club prior.
      const faceOn = estimateDistance(
        track,
        { club: 'driver', cameraAngle: 'face-on', horizontalFovDeg: 60 },
        portrait,
      );
