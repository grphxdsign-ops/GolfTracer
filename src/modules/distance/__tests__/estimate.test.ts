...
describe('estimateDistance', () => {
  ...
  it(
    "uses the DTL rung ('physics-fit' + dtlFit) on a 12-point behind track",
    () => {
      // Same ladder scenario as the old receding-rung test — a 12-point
      // 1080x1920 portrait 30 fps behind view where the planar fit is
      // geometrically hopeless — updated to the DTL 3D fit's semantics:
      // the metric anchor is the ball radius at address, the tee ray comes
      // from the user's ball tap, and the honesty assertions are unchanged
      // (measured carry near truth, confidence in the DTL band, identical
      // face-on track still falls to the club prior).
      ...
      const scene = makeSyntheticDtlTrack({
        ...
        teeDistanceM: 4,
        hfovDeg: 44,
        frameWidth: portrait.width,
        frameHeight: portrait.height,
        fps: portrait.fps,
        nObs: 12,
        noisePx: 1.5,
        seed: 42,
      });
      const trueCarry = scene.trueCarryYards;

      const estimate = estimateDistance(
        scene.track,
        {
          club: 'driver',
          cameraAngle: 'behind',
      ...
      expect(estimate.method).toBe('physics-fit');
      expect(estimate.dtlFit).toBeDefined();
      expect(estimate.dtlFit!.converged).toBe(true);
      // The deprecated receding rung is never populated anymore.
      expect(estimate.recedingFit).toBeUndefined();
      // Honest confidence: above the club-prior cap, inside the DTL band.
      expect(estimate.confidence).toBeGreaterThan(0.3);
      expect(estimate.confidence).toBeLessThanOrEqual(0.75);
      expect(
        Math.abs(estimate.carryYards - trueCarry) / trueCarry,
      ).toBeLessThan(0.2);

      // The identical track calibrated face-on has no DTL rung and no
      // converging planar fit — it must fall to the honest club prior.
      const faceOn = estimateDistance(
        scene.track,
        { club: 'driver', cameraAngle: 'face-on', horizontalFovDeg: 44 },
        portrait,
      );
      expect(faceOn.method).toBe('club-prior');
      expect(faceOn.confidence).toBeLessThanOrEqual(0.3);

      // Ladder confidence ranking: homography sits above the DTL band
      // (> 0.75), the DTL fit above the prior band (> 0.3 vs <= 0.3).
      expect(estimate.confidence).toBeGreaterThan(faceOn.confidence);
    },
    60000,
  );
  ...
  it('confidence ranks homography above club-prior for the same quality', () => {
    ...
    // Ladder ranking: a clean homography measurement outranks even the best
    // possible DTL fit (capped at 0.75), which outranks the prior (<= 0.3).
    expect(withRefs.confidence).toBeGreaterThan(0.75);
    expect(withoutRefs.confidence).toBeLessThanOrEqual(0.3);
  });
});
